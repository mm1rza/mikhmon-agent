...
function processCommand($phone, $message) {
    ...
    if (isAdminNumber($phone)) {
        ...
        // Command: GANTI WIFI <IDENTIFIER> <SSID_BARU> - Ganti SSID WiFi customer (Admin only)
        // Example: GANTI WIFI 081234567890 MyWiFi
        elseif (strpos($messageLower, 'ganti wifi ') === 0) {
            $rest = trim(str_replace('ganti wifi ', '', $messageLower));
            $parts = preg_split('/\s+/', $rest, 2);
            
            if (count($parts) >= 2) {
                $identifier = trim($parts[0]); // Phone, service number, or PPPoE username
                $ssid = trim($parts[1]);
                changeCustomerWiFiSSIDByAdmin($phone, $identifier, $ssid);
            } else {
                sendWhatsAppMessage($phone, "❌ *FORMAT SALAH*\n\nFormat: GANTI WIFI <NOMOR/USERNAME> <SSID_BARU>\nContoh: GANTI WIFI 081234567890 MyWiFi");
            }
            return;
        }
        // Command: GANTI SANDI <IDENTIFIER> <PASSWORD_BARU> - Ganti password WiFi customer (Admin only)
        // Example: GANTI SANDI 081234567890 password123
        elseif (strpos($messageLower, 'ganti sandi ') === 0) {
            ...
            if (count($parts) >= 2) {
                ...
                sendWhatsAppMessage($phone, "❌ *FORMAT SALAH*\n\nFormat: GANTI SANDI <NOMOR/USERNAME> <PASSWORD_BARU>\nContoh: GANTI SANDI 081234567890 password123");
            }
            return;
        }
    }
...
function changeCustomerWiFiSSIDByAdmin($adminPhone, $identifier, $ssid) {
    // Validate SSID (max 32 characters)
    if (strlen($ssid) > 32) {
        sendWhatsAppMessage($adminPhone, "❌ *SSID TERLALU PANJANG*\n\nSSID maksimal 32 karakter.");
        return;
    }
    ...
    try {
        ...
        // Change SSID only
        $result = $billingService->changeCustomerWifi((int)$customer['id'], $ssid, null);
        ...
        if ($result['success']) {
            ...
            // Log the change
            logWebhook("SSID changed by admin for customer {$customer['name']} (ID: {$customer['id']}) - SSID: {$ssid}");
        } else {
            $errorMsg = $result['message'] ?? 'Gagal mengubah SSID';
